...
def fairy_tale_book(request):
    if request.method == 'GET':
        fairy_tale_book = models.Book.objects.filter(tags_name='Сказка')
...
def fantastic_book(request):
    if request.method == 'GET':
        fantastic_book = models.Book.objects.filter(tags_name='Фантастика')
...
def drama_book(request):
    if request.method == 'GET':
        drama_book = models.Book.objects.filter(tags_name='Драмма')
